...
def mle_fit(data, init_D=1.0, init_b=1e-2, r_max=0.8, dt=0.020):
    ...
    p0 = (init_D, init_b)
    res = minimize(_obj, p0, args=(data, r_max, dt),
                   method="Nelder-Mead",
...
def isolate_spots(data, mask=None, threshold=10, sigma=1.5):
    ...
    for t in tqdm(range(Nt)):
        ...
        _peaklocs = peak_local_max(_response, labels=mask, threshold_abs=threshold,
                                   min_distance=3)
...
def link_spots_to_trajectory(spots, max_displacement=10.0):
    ...
    """
    tracker = LapTrack(track_cost_cutoff=max_displacement**2,
...
def compute_msd(sdf):
    ...
    if nframes > 1:
        ...
        for i, lag in enumerate(lags):
            ...
            sqdist = (
                np.square(xc[s2] - xc[s1]) + 
                np.square(yc[s2] - yc[s1])
            )
